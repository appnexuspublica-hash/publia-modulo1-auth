...
export default function OfficialGazetteClient() {
  ...
  const [syncActionKey, setSyncActionKey] = useState<string | null>(null);
  ...
  async function loadDocuments(gazetteId: string | null) {
    ...
    try {
      ...
      const nextDocuments: OfficialGazetteDocumentRow[] = payload.documents ?? [];
  ...
  async function handleSyncSelectedGazette() {
    if (!selectedGazette) {
      setErrorMessage("Selecione uma fonte do Diário Oficial para sincronizar.");
      return;
    }
    ...
    setDocumentErrorMessage(null);
    setDocumentSuccessMessage(null);
    setSyncActionKey(selectedGazette.id);

    try {
      const response = await fetch("/api/governance/official-gazette/sync", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        throw new Error(
          payload?.error ?? "Não foi possível sincronizar o Diário Oficial.",
        );
      }
      ...
      setSuccessMessage(
        `Sincronização concluída. Novas edições: ${created}. Já cadastradas: ${skipped}. Erros: ${errors}.`,
      );
  ...
  function handleDocumentSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!title) {
      setDocumentErrorMessage("Informe o título da edição.");
      return;
    }
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
              Cadastre a URL oficial onde a organização publica o Diário Oficial.
              Depois, use a sincronização automática para buscar novas edições
              publicadas, baixar os PDFs oficiais e indexar os atos.
            </p>
          </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="space-y-6">
          ...
          >
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              ...
              <button
                type="button"
                onClick={handleSyncSelectedGazette}
                disabled={!selectedGazette || syncActionKey === selectedGazette.id}
                className="inline-flex items-center justify-center gap-2 rounded-2xl border border-[#dedede] bg-[#f8f8f8] px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-70"
                title="Buscar novas edições publicadas na fonte oficial."
              >
                {syncActionKey === selectedGazette?.id ? (
                  <Loader2 size={18} className="animate-spin" />
                ) : (
                  <Search size={18} />
                )}
          ...
          <section className="rounded-3xl border border-[#dedede] bg-white p-6 shadow-sm">
            <div className="mb-5">
              ...
              <p className="mt-1 text-sm text-slate-600">
                Cadastre manualmente uma edição publicada quando a sincronização
                automática não encontrar o PDF no site oficial.
              </p>
            </div>
              ...
              <div className="grid gap-4 lg:grid-cols-3">
                <label className="block lg:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="Diário Oficial nº 2485"
        ...
        <aside className="space-y-6">
          <section className="flex max-h-[calc(100vh-180px)] flex-col overflow-hidden rounded-3xl border border-[#dedede] bg-white p-6 shadow-sm">
            <div className="mb-5">
              <h2 className="text-lg font-bold text-slate-950">
                Edições cadastradas
              </h2>
                      ...
                      <div className="flex flex-col gap-3">
                        <div>
                          <div className="flex items-center gap-2">
                            <FileText size={16} className="text-[#0f3a4a]" />
                          ...
                          <p className="mt-2 text-sm text-slate-600">
                            Edição: {document.edition_number || "Não informada"} ·
                            Publicação: {formatDate(document.publication_date)}
                          </p>
                        ...
                        <div className="flex flex-wrap gap-2 border-t border-[#dedede] pt-3">
                          <button
                            type="button"
                            onClick={() => handleToggleDocumentChunks(document)}
                            disabled={documentActionKey !== null || isLoadingChunks}
                            className="inline-flex items-center justify-center gap-1 rounded-xl border border-[#dedede] bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-[#f8f8f8] disabled:cursor-not-allowed disabled:opacity-60"
                          >
                            {isLoadingChunks ? (
                          ...
                          <button
                            ...
                            disabled={documentActionKey !== null}
                            className="inline-flex items-center justify-center gap-1 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60"
                          >
