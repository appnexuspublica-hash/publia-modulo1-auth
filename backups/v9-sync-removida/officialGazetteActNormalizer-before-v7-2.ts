...
function detectSectionType(title: string) {
  ...
  if (normalizedTitle.includes("LEI MUNICIPAL")) return "lei";
...
function getOfficialGazetteActHeaderPattern() {
  return new RegExp(
    [
      ...
      // "Lei nº ..." e "Lei Municipal nº ..." não entram aqui nesta etapa,
      // pois aparecem com frequência como fundamento/citação dentro de outros atos.
      String.raw`\bATA\s+DA\s+ABERTURA[\s\S]{0,320}?DISPENSA\s+DE\s+LICITA[ÇC][ÃA]O\s+[\d.]+(?:[/-]\d{2,4})?`,
      ...
      String.raw`\bEXTRATO\s+DE\s+(?:CONTRATO|TERMO|ATA)\b[A-ZÁÀÂÃÉÊÍÓÔÕÚÇ Nº°O./-]{0,120}`,
...
function getCanonicalKey(title: string, content: string) {
  ...
  const decreeNumber = normalizeNumber(
    extractNumberAfter(/DECRETO\s+(?:MUNICIPAL\s+)?N[º°O.]?\s*([\d.]+(?:[/-]\d{2,4})?)/i, source),
  );
  if (decreeNumber) return `decreto:${decreeNumber}`;

  const portariaNumber = normalizeNumber(
    extractNumberAfter(/PORTARIA\s+N[º°O.]?\s*([\d.]+(?:[/-]\d{2,4})?)/i, source),
  );
  if (portariaNumber) return `portaria:${portariaNumber}`;

  const resolucaoNumber = normalizeNumber(
    extractNumberAfter(/RESOLU[ÇC][ÃA]O\s+(?:N[º°O.]?\s*)?([\d.]+(?:[/-]\d{2,4})?)/i, source),
  );
  if (resolucaoNumber) return `resolucao:${resolucaoNumber}`;

  const leiMunicipalNumber = normalizeNumber(
    extractNumberAfter(/LEI\s+MUNICIPAL\s+N[º°O.]?\s*([\d.]+(?:[/-]\d{2,4})?)/i, source),
  );
  if (leiMunicipalNumber) return `lei-municipal:${leiMunicipalNumber}`;
