...
function governanceReferenceAppearsInAnswer(
  ...
  const exactNumberedMatch = numberedReferenceAppearsExactly(reference, answer);

  if (exactNumberedMatch !== null) {
    return exactNumberedMatch;
  }
...
function isLicitationEvidenceQuestion(question: string, answer: string) {
  const questionText = normalizeReferenceTextForMatch(question);
  const answerText = normalizeReferenceTextForMatch(answer);
  const isLicitationTopic = referenceIncludesAny(
  ...
  if (!isLicitationTopic) {
    return false;
  }

  /*
    v13.15 — uma pergunta meramente explicativa sobre licitações não é,
    por si só, um pedido de evidências municipais. O modo documental só é
    ativado quando o usuário pede consulta, localização, listagem, publicação
    ou identificação de atos concretos.
  */
  return referenceIncludesAny(questionText, [
    "onde consultar",
    "onde encontrar",
    "onde foi publicado",
    "onde foi publicada",
...
function isGenericOfficialReference(reference: GovernanceChatReference) {
  const title = normalizeReferenceTextForMatch(reference.title);
  ...
  return [
    ...
    "portal da transparencia",
    "portal transparencia betha contabil",
    "site oficial",
  ].includes(title);
}

function extractActIdentifiers(value: unknown) {
  const text = String(value ?? "");
  const identifiers = new Set<string>();

  for (const match of text.matchAll(
    /\b(?:lei(?:\s+complementar)?|decreto|portaria|resolu[cç][aã]o|preg[aã]o(?:\s+eletr[oô]nico)?|extrato(?:\s+de\s+contrato)?|contrato|edi[cç][aã]o)\s*(?:n[º°.]?\s*)?(\d{1,6})(?:\s*\/\s*(\d{4}))?/gi,
  )) {
    const number = String(match[1] ?? "").replace(/^0+/, "") || "0";
    const year = String(match[2] ?? "");
    identifiers.add(year ? `${number}/${year}` : number);
  }

  return identifiers;
}

function numberedReferenceAppearsExactly(
  reference: GovernanceChatReference,
  answer: string,
) {
  const identifiers = extractActIdentifiers(reference.title);

  if (identifiers.size === 0) {
    return null;
  }

  const answerIdentifiers = extractActIdentifiers(answer);

  return Array.from(identifiers).some((identifier) =>
    answerIdentifiers.has(identifier),
  );
}



function isSoftwareProcurementQuestion(question: string) {
  const text = normalizeReferenceTextForMatch(question);
...
function isSoftwareProcurementReference(reference: GovernanceChatReference) {
  const text = normalizeReferenceTextForMatch(
    `${reference.title} ${reference.supportText ?? ""}`,
  );

  const related = referenceIncludesAny(text, [
    "software",
  ...
  const unrelated = referenceIncludesAny(text, [
    "sondagem spt",
    "ensaio cbr",
    "viga benkelman",
    "uniformes escolares",
...
function isNegativeEvidenceAnswer(answer: string) {
  const text = normalizeReferenceTextForMatch(answer);
...
function shouldKeepGovernanceReferenceForClient(params: {
  ...
  if (!String(reference.url ?? "").trim()) {
    return false;
  }

  if (
    isSoftwareProcurementQuestion(question) &&
    !isSoftwareProcurementReference(reference)
  ) {
    return false;
  }

  const explicitSourceQuestion =
    isOfficialSourcesEvidenceQuestion(question, "") ||
    isLicitationEvidenceQuestion(question, "");

  if (isGenericOfficialReference(reference) && !explicitSourceQuestion) {
    return false;
  }

  /*
    v13.18 — numa resposta negativa, citar documentos recuperados apenas para
    dizer que eram irrelevantes não os transforma em fontes da conclusão.
  */
  if (isNegativeEvidenceAnswer(answer)) {
    /*
      v13.20 — uma resposta negativa não possui fonte documental positiva.
      Documentos apenas varridos ou descartados não provam a inexistência do
      fato consultado e, por isso, não devem aparecer no rodapé.
    */
    return false;
  }
...
export function filterGovernanceChatReferencesForClient(
  ...
  if (isLicitationEvidenceQuestion(question, answer)) {
    return [
      ...officialGazetteReferences.slice(0, 2),
      ...licitationOfficialReferences.slice(0, 2),
      ...otherReferences.slice(0, 1),
    ].slice(0, 5);
  }

  /*
    v13.15 — em respostas explicativas, "Fontes consultadas" é um rodapé
    enxuto de evidências específicas, não um inventário de todos os portais
    recuperados durante a busca.
  */
