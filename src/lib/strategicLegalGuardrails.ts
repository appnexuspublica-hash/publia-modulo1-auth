 ...
 *
 * O Estratégico reutiliza a base crítica consolidada do projeto, mas normaliza
...
function expandStrategicAliases(userText: string) {
...
function buildStrategicSpecificRules(userText: string) {
  const normalized = normalize(userText);
  const rules: string[] = [];
  ...
  return [
    "REGRAS ESPECÍFICAS DO PUBL.IA ESTRATÉGICO",
    "",
    ...rules,
  ].join("\n\n");
}

export function buildStrategicLegalGuardrails(userText: string): string {
  const expandedText = expandStrategicAliases(userText);
  const sharedRules = buildGovernanceLegalGuardrails(expandedText);
  const essentialRules = buildStrategicSpecificRules(userText);
