...
type GovernanceModeHeadingLevel = "title" | "section" | "subsection";

const governanceResponseModes = new Set<GovernanceResponseMode>(
  responseModeOptions.map((item) => item.value),
);

function getMessageResponseMode(
  message: GovernanceMessage,
): GovernanceResponseMode {
  const rawMode = String(message.metadata?.response_mode ?? "").trim();

  return governanceResponseModes.has(rawMode as GovernanceResponseMode)
    ? (rawMode as GovernanceResponseMode)
    : "objective";
}

function normalizeModeHeadingText(value: string) {
  return String(value ?? "")
    .replace(/[*_`#]+/g, "")
    .replace(/\s+/g, " ")
    .trim()
...
function getModeHeadingLevel(
  mode: GovernanceResponseMode,
  value: string,
): GovernanceModeHeadingLevel | null {
  const text = normalizeModeHeadingText(value);

  if (!text || text.length > 110) {
    return null;
  }

  const titlePatterns: Partial<Record<GovernanceResponseMode, RegExp>> = {
    summary: /^resumo(?: executivo)?$/,
    checklist: /^checklist(?: prático)?$/,
    technical_opinion: /^parecer técnico$/,
    legal_opinion: /^parecer jurídico$/,
    risk_analysis: /^(?:análise|matriz) de riscos?$/,
    attention_points: /^pontos de atenção$/,
    action_plan: /^plano de ação$/,
    draft: /^minuta(?: .+)?$/,
    comparison: /^(?:quadro |análise )?comparativ[oa]$/,
  };

  if (titlePatterns[mode]?.test(text)) {
    return "title";
  }

  const sectionPatterns: Record<GovernanceResponseMode, RegExp> = {
    objective:
      /^(?:[1-5][.)]\s*)?(?:resposta objetiva|fundamentação|entendimento atual|aplicação ao município|cuidados)$/,
    summary:
      /^(?:resposta direta|síntese|principais pontos|conclusão(?: prática)?)$/,
    manager_guidance:
      /^(?:decisão necessária|o que observar|providências recomendadas|riscos (?:da inação|se nada for feito)|próximo passo(?: sugerido)?)$/,
    checklist:
      /^(?:etapa|tema|documentos|verificações|ações recomendadas|observações)(?:\s+.+)?$/,
    technical_opinion:
      /^(?:[1-6][.)]\s*)?(?:assunto|relatório|fundamentação técnica|análise|conclusão|recomendação técnica)$/,
    legal_opinion:
      /^(?:ementa|identificação|[ivxlcdm]+[.)\s–-]+(?:relatório|delimitação da consulta|fundamentação jurídica|análise do caso concreto|riscos jurídicos e condicionantes|conclusão|recomendações)|assinatura)$/,
    risk_analysis:
      /^(?:matriz de riscos?|análise|impactos?|medidas mitigadoras|mitigação|prioridades de controle|resposta objetiva à pergunta(?: contextualizada no risco)?)$/,
    attention_points:
      /^(?:atenção(?:\s+\d+)?|providências|validações|recomendações)$/,
    action_plan:
      /^(?:objetivo|prioridades imediatas|responsáveis|prazo sugerido|resultado esperado)$/,
    draft:
      /^(?:interessado|assunto|objeto|considerando|resolve|cláusula(?:\s+\w+)?|conclusão|local e data|assinatura)$/,
    comparison:
      /^(?:critérios?|conclusão comparativa|recomendação)$/,
  };

  if (sectionPatterns[mode].test(text)) {
    return "section";
  }

  if (
    mode === "legal_opinion" &&
    /^(?:processo nº|interessado|órgão consulente|assunto)\b/.test(text)
  ) {
    return "subsection";
  }
...
function getModeHeadingClass(
  mode: GovernanceResponseMode,
  level: GovernanceModeHeadingLevel,
) {
  if (level === "title") {
    if (mode === "legal_opinion" || mode === "draft") {
      return "mb-5 mt-1 border-b border-slate-300 pb-3 text-center text-lg font-black uppercase tracking-[0.08em] text-slate-950";
    }

    if (mode === "technical_opinion") {
      return "mb-5 mt-1 border-b border-slate-300 pb-3 text-lg font-black uppercase tracking-[0.06em] text-slate-950";
    }

    return "mb-4 mt-1 text-lg font-black uppercase tracking-[0.06em] text-[#0f3a4a]";
  }

  if (level === "subsection") {
    return "mb-1 mt-3 text-sm font-bold text-slate-900";
  }

  if (mode === "legal_opinion") {
    return "mb-3 mt-7 border-b border-slate-200 pb-2 text-[15px] font-black uppercase tracking-[0.04em] text-slate-950";
  }

  if (mode === "technical_opinion") {
    return "mb-3 mt-7 border-b border-slate-200 pb-2 text-[15px] font-black uppercase tracking-[0.03em] text-slate-950";
  }

  if (mode === "manager_guidance") {
    return "mb-3 mt-7 border-b border-slate-200 pb-2 text-base font-black text-[#0f3a4a]";
  }
  ...
  if (
    ...
    return "mb-3 mt-6 text-[15px] font-black uppercase tracking-wide text-[#0f3a4a]";
  }

  if (mode === "attention_points") {
    return "mb-3 mt-6 border-l-4 border-[#0f3a4a] pl-3 text-[15px] font-black text-slate-950";
  }

  return "mb-3 mt-6 text-base font-black text-[#0f3a4a]";
}

function getModeContainerClass(mode: GovernanceResponseMode) {
  if (mode === "legal_opinion" || mode === "draft") {
    return "governance-answer-document";
  }
  ...
  if (
    ...
    return "governance-answer-structured";
  }

  return "governance-answer-consultive";
}
...
function removeNoWebConsultationNoise(content: string) {
  return String(content ?? "")
    .replace(
      /(?:^|\n)\s*\*\*?Refer[eê]ncias oficiais consultadas:\*\*?\s*\n\s*[-•]\s*N[aã]o houve consulta web[^\n]*(?=\n|$)/gi,
      "",
    )
    .replace(
      /(?:^|\n)\s*Refer[eê]ncias oficiais consultadas:\s*\n\s*[-•]\s*N[aã]o houve consulta web[^\n]*(?=\n|$)/gi,
...
export default function GovernanceChatClient({
  ...
  useEffect(() => {
    // Cada conversa/pergunta nova deve iniciar no modo Padrão.
    // O modo escolhido vale somente para a requisição atual.
  ...
  function sendContent(
    ...
    const responseModeForRequest = selectedResponseMode;
...
<html>
  ...
  function getMessageSources(message: GovernanceMessage): GovernanceChatSources {
  ...
  function normalizeInstitutionalSources(
    ...
    const unique = new Map<string, Required<Pick<GovernanceChatSource, "title">> & GovernanceChatSource>();
  ...
  function renderAssistantContent(message: GovernanceMessage) {
    ...
    const institutionalSourcesForMessage = normalizeInstitutionalSources(
    ...
    function renderMarkdownBlock(markdownContent: string) {
      ...
      return (
        <div
          ...
          className={[
            "governance-answer prose max-w-none prose-slate text-[15px] leading-8 prose-headings:font-bold prose-headings:text-slate-950 prose-p:my-3 prose-strong:font-bold prose-strong:text-slate-950 prose-ul:my-3 prose-ol:my-3 prose-li:my-1 prose-li:pl-1 prose-hr:my-5 prose-hr:border-[#dedede]",
            getModeContainerClass(responseMode),
          ].join(" ")}
        >
          <ReactMarkdown
            ...
            components={{
              ...
              h1: ({ node, ...props }) => (
                <h1
                  {...props}
                  className={getModeHeadingClass(responseMode, "title")}
                />
              ),
              h2: ({ node, ...props }) => (
                <h2
                  {...props}
                  className={getModeHeadingClass(responseMode, "section")}
                />
              ),
              h3: ({ node, ...props }) => (
                <h3
              ...
              h4: ({ node, ...props }) => (
                <h4
                  {...props}
                  className={getModeHeadingClass(responseMode, "subsection")}
                />
              ),
              p: ({ node, ...props }) => {
                ...
                const isCompactLegalBasis = /^base legal\s*:/i.test(text);

                if (isCompactLegalBasis) {
                  return (
                    <div className="my-0 text-sm leading-6 text-slate-800">
                      {props.children}
                    </div>
                  );
                }

                const modeHeadingLevel = getModeHeadingLevel(responseMode, text);

                if (modeHeadingLevel === "title") {
                  return (
                    <h1 className={getModeHeadingClass(responseMode, "title")}>
                      {props.children}
                    </h1>
                  );
                }

                if (modeHeadingLevel === "section") {
                  return (
                    <h2 className={getModeHeadingClass(responseMode, "section")}>
                      {props.children}
                    </h2>
                  );
                }

                if (modeHeadingLevel === "subsection") {
                  return (
                    <h3 className={getModeHeadingClass(responseMode, "subsection")}>
                      {props.children}
                    </h3>
                  );
                }
                ...
                if (isHighlightedParagraph) {
                  if (responseMode === "attention_points") {
                    return (
                      <div className="my-4 border-l-4 border-[#0f3a4a] pl-4 text-[15px] leading-8 text-slate-800">
                        {props.children}
                      </div>
                    );
                  }
              ...
              table: ({ node, ...props }) => (
                <div
                  className={[
                    "my-4 overflow-x-auto border border-[#dedede] bg-white",
          ...
          >
            {normalizeAssistantMarkdown(markdownContent, institutionalSourcesForMessage)}
          </ReactMarkdown>
        </div>
      );
    }
    ...
    <div className="flex h-screen flex-col bg-[#f5f5f5] text-slate-900">
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <main className="flex min-w-0 flex-1 flex-col bg-[#f8f8f8]">
          ...
          <div className="shrink-0 border-t border-[#dedede] bg-[#f8f8f8] px-4 py-3">
            <GovernanceFooter variant="chat" />
          </div>
        </main>
      </div>
    </div>
