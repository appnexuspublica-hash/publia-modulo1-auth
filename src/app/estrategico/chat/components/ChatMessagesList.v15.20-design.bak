//src/app/estrategico/chat/components/ChatMessagesList.tsx
"use client";
...
const STRATEGIC_FOOTER_TEXT_STYLE: CSSProperties = {
  fontSize: "0.8125rem",
  lineHeight: 1.5,
};

const STRATEGIC_FOOTER_HEADING_STYLE: CSSProperties = {
  fontSize: "0.8125rem",
  lineHeight: 1.5,
  fontWeight: 600,
};
...
const THEME_CHAT_STRATEGIC: MdTheme = {
  tableOuterBorder: "w-full overflow-hidden rounded-xl border border-[#0D2B4D]/20",
  tableClassWide: "min-w-max w-full border-collapse text-[13px] text-slate-800",
  tableClassNormal: "w-full table-auto border-collapse text-[13px] text-slate-800",
  theadBg: "bg-[#0D2B4D]/8",
  thWide:
    "whitespace-nowrap border border-[#0D2B4D]/15 px-3 py-2 text-left font-semibold text-[#0D2B4D]",
  thNormal:
    "whitespace-normal break-words border border-[#0D2B4D]/15 px-3 py-2 text-left font-semibold text-[#0D2B4D]",
  tdWide:
    "whitespace-nowrap border border-[#0D2B4D]/10 px-3 py-2 align-top text-slate-800",
  tdNormal:
    "whitespace-normal break-words border border-[#0D2B4D]/10 px-3 py-2 align-top text-slate-800",
  btn: "rounded-full border border-[#0D2B4D]/30 px-3 py-1 text-[11px] font-medium text-[#0D2B4D] transition hover:bg-[#0D2B4D]/5",
  pre: "my-3 overflow-x-auto rounded-xl bg-[#0D2B4D] p-3 text-white",
  link: "font-medium text-[#0D2B4D] underline underline-offset-2 hover:opacity-80",
};
...
function splitMarkdownFooter(md: string): { main: string; footer: string } {
  ...
  // O backend pode entregar um ou mais separadores Markdown antes de
  // "Base legal". Removemos todos daqui e desenhamos um único separador
  // visual no container do rodapé.
  const main = s
    .slice(0, start)
    .replace(/(?:\n\s*(?:-{3,}|\*{3,}|_{3,})\s*)+$/g, "")
    .trimEnd();
        ...
        {messages.map((msg) => {
          ...
          const markdownComponentsFooter: any = {
            ...
            h1: ({ node, ...props }: any) => {
              ...
              return (
                <p
                  className="publia-footnote__heading"
                  style={STRATEGIC_FOOTER_HEADING_STYLE}
                >
            ...
            ul: ({ node, style, ...props }: any) => (
              <ul
                {...props}
                className="mt-2 list-none space-y-1 p-0"
                style={{ ...STRATEGIC_FOOTER_TEXT_STYLE, ...style }}
              />
            ),
            ol: ({ node, style, ...props }: any) => (
              <ol
                {...props}
                className="mt-2 list-none space-y-1 p-0"
                style={{ ...STRATEGIC_FOOTER_TEXT_STYLE, ...style }}
            ...
            li: ({ node, style, ...props }: any) => (
              <li
                className="flex gap-2"
                style={{ ...STRATEGIC_FOOTER_TEXT_STYLE, ...style }}
              >
                <span className="shrink-0" style={STRATEGIC_FOOTER_TEXT_STYLE}>
                  -
                </span>
                <span className="min-w-0" style={STRATEGIC_FOOTER_TEXT_STYLE}>
                  {props.children}
                </span>
              </li>
            ),
                ...
                <div className="flex flex-col items-start">
                  ...
                  >
                    ...
                    {footer.trim().length > 0 && (
                      <div
                        className={"publia-footnote publia-footnote--strategic mt-4 border-t pt-4 " + assistantMetaTextClass}
                        data-footnote
                        style={
                          variant === "chat" && isStrategic
                            ? {
                          ...
                          <button
                            ...
                            style={
                              isStrategic && variant === "chat"
                                ? {
                                    borderColor: chatTheme.colors.borderStrong,
                                    backgroundColor: chatTheme.colors.bgTertiary,
                                    color: chatTheme.colors.text,
                                  }
                                : undefined
                            }
