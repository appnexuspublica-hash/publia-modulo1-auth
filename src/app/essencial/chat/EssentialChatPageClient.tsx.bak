// src/app/essencial/chat/EssentialChatPageClient.tsx
"use client";
...
import { getChatTheme } from "@/app/essencial/chat/theme";
...
type EssentialChatPageClientProps = {
  userId: string;
  userLabel: string;
  /**
   * Endpoint exclusivo do Publ.IA Essencial.
   */
...
export default function EssentialChatPageClient({
  userId,
  userLabel,
  chatApiEndpoint = "/api/essential/chat",
}: EssentialChatPageClientProps) {
  ...
  const responseMode: ResponseMode = "objective";
  ...
  }, [attachedPdfs, effectiveSelectedPdfIds]);
  ...
  const pdfUploadMaxMb = ESSENTIAL_PDF_UPLOAD_MAX_MB;
  const isStrategic = false;
  const theme = useMemo(() => getChatTheme(false), []);
  ...
  const strategicResponseModes: ResponseMode[] = [];
  const showResponseModes = false;
  const strategicSuggestions: SuggestedNextQuestion[] = [];
  ...
  const loadConversationPdfs = useCallback(
    async (conversationId: string) => {
      ...
      setSelectedPdfIds((prev) => {
        ...
        // Preserva a seleção explícita do usuário. PDFs recém-enviados são
        // adicionados à seleção antes da recarga da conversa.
        return filtered;
      });
  ...
  async function createConversation(
    ...
    try {
      const { data, error } = await supabase
        .from("conversations")
        .insert({ user_id: userId, title: "Nova conversa", product_tier: "essential" })
        .select("id, title, created_at, is_shared, share_id, is_favorite")
        .single();
      ...
      while (true) {
        ...
        while ((idx = buffer.indexOf("\n\n")) !== -1) {
          ...
          if (parsed.event === "meta") {
            ...
            if (serverProductTier === "essential") {
              setAccess((prev) =>
                prev
                  ? {
                      ...prev,
            ...
            void serverAllowedModes;
            void serverEffectiveMode;
          }
          ...
          if (parsed.event === "error") {
            ...
            void apiAllowedModes;

            if (responseModeBlocked) {
              console.warn(
                "A API do Essencial rejeitou um modo de resposta não suportado."
              );
            }
