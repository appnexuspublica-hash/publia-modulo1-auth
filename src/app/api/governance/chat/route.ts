...
function buildGovernanceModeInstruction(mode: GovernanceResponseMode): string {
  switch (mode) {
    ...
    default:
      return [
        "Modo institucional selecionado: PADRÃO CONSULTIVO.",
        "Este é o modo principal do chat. Ele deve priorizar orientação consultiva, fluida e didática.",
        "Não responda como checklist, parecer, relatório ou manual, salvo pedido expresso do usuário.",
        "Explique a lógica primeiro, em texto corrido, e só depois organize os pontos práticos.",
        "A resposta deve ajudar o gestor a entender o caminho correto e o motivo de cada providência.",
      ].join("\n");
  }
}
...
function buildGovernanceConsultantStyleInstruction(): string {
  return [
...
function buildFinalConsultativeOverride(mode: GovernanceResponseMode): string {
  if (mode !== "objective") {
    return [
      "REGRA FINAL DE ESTILO",
      "Mesmo respeitando o modo selecionado, preserve linguagem clara, humana e consultiva.",
      "Não transforme a resposta em blocos burocráticos desnecessários.",
    ].join("\n");
  }
...
export async function POST(request: Request) {
  try {
    ...
    const instructions = [
      ...
      "",
      buildFinalConsultativeOverride(conversation.response_mode),
