// src/app/estrategico/chat/StrategicChatPageClient.tsx
"use client";
...
import { getChatTheme } from "@/app/estrategico/chat/theme";
...
type StrategicChatPageClientProps = {
  userId: string;
  userLabel: string;
  /**
   * Endpoint usado para envio das mensagens.
   * Endpoint exclusivo do Publ.IA Estratégico.
   */
...
export default function StrategicChatPageClient({
  userId,
  userLabel,
  chatApiEndpoint = "/api/strategic/chat",
}: StrategicChatPageClientProps) {
  ...
  const pdfSelectionInitializedForConversationRef = useRef<string | null>(null);

  const effectiveSelectedPdfIds = useMemo(() => {
    const attachedIds = new Set(attachedPdfs.map((pdf) => pdf.id));
    return selectedPdfIds.filter((id) => attachedIds.has(id));
  ...
  const pdfsBlockingChat = useMemo(() => {
    ...
    const selectedSet = new Set(effectiveSelectedPdfIds);
    const candidates =
      selectedSet.size > 0
  ...
  const strategicButtonClassName =
    "border text-[#0D2B4D] transition duration-150 hover:bg-[#E5E5E5]";
  ...
  const strategicHintButtonClassName =
    "rounded-full border px-3 py-2 text-[11px] font-semibold text-[#0D2B4D] transition duration-150 hover:bg-[#E5E5E5]";
  const strategicMenuItemClassName =
    "block w-full px-4 py-2 text-left transition hover:bg-[#E5E5E5]";
  ...
  const strategicSuggestions = useMemo(() => {
    ...
    const hasContextualPdfs =
      attachedPdfs.length > 0 && effectiveSelectedPdfIds.length > 0;
    ...
    responseMode,
    attachedPdfs.length,
    effectiveSelectedPdfIds.length,
  ]);
  ...
  const loadConversationPdfs = useCallback(
    async (conversationId: string) => {
      ...
      setSelectedPdfIds((prev) => {
        ...
        const isInitialLoadForConversation =
          pdfSelectionInitializedForConversationRef.current !== conversationId;

        if (isInitialLoadForConversation) {
          pdfSelectionInitializedForConversationRef.current = conversationId;
          return validIds;
        }

        // Preserva a escolha explícita do usuário, inclusive quando todos
        // os PDFs foram desmarcados. Novos PDFs são adicionados à seleção
        // no fluxo de upload antes desta recarga.
        return filtered;
      });
  ...
  useEffect(() => {
    if (!activeConversationId) {
      pdfSelectionInitializedForConversationRef.current = null;
      setMessages([]);
      setAttachedPdfs([]);
      setActivePdfId(null);
      setSelectedPdfIds([]);
  ...
  function getEffectivePdfSelectionForChat() {
    ...
    return {
      ...
      selectedPdfIds: effectiveSelectedPdfIds,
    };
  }
  ...
  async function handleQuickPdfAction(kind: QuickActionKind) {
    if (!activeConversationId || effectiveSelectedPdfIds.length === 0) {
      showToast("Selecione pelo menos um PDF antes de usar uma ação rápida.", {
        type: "warning",
        persistent: true,
      });
      return;
    ...
    const selectedPdfs = attachedPdfs.filter((pdf) => selectedSet.has(pdf.id));
    const selectedNames = selectedPdfs.map((pdf) => pdf.fileName).filter(Boolean);
    const isMultiple = selectedPdfs.length > 1;
    const documentsLabel = selectedNames.length
      ? selectedNames.map((name) => `- ${name}`).join("\n")
      : "- PDF selecionado";

    const prompts: Record<QuickActionKind, string> = isMultiple
      ? {
          resumo: [
            "Faça um resumo executivo conjunto dos PDFs selecionados.",
            "Apresente os pontos principais de cada documento e, ao final, destaque convergências, divergências e relações entre eles.",
            "",
            "PDFs selecionados:",
            documentsLabel,
          ].join("\n"),
          pontos: [
            "Analise conjuntamente os PDFs selecionados e liste os principais pontos de atenção, obrigações, prazos, riscos e cuidados.",
            "Separe o que pertence a cada documento e destaque também os pontos comuns ou conflitantes.",
            "",
            "PDFs selecionados:",
            documentsLabel,
          ].join("\n"),
          irregularidade: [
            "Analise conjuntamente os PDFs selecionados e aponte possíveis inconsistências, irregularidades, contradições ou pontos que exigem validação.",
            "Indique em qual documento cada ponto foi identificado e destaque conflitos entre os documentos.",
            "",
            "PDFs selecionados:",
            documentsLabel,
          ].join("\n"),
      ...
      : {
          resumo: [
            "Faça um resumo executivo objetivo do PDF selecionado, destacando os pontos principais em tópicos.",
            "",
            "PDF selecionado:",
            documentsLabel,
          ].join("\n"),
          pontos: [
            "Liste os principais pontos de atenção, obrigações, prazos, riscos ou cuidados do PDF selecionado em tópicos.",
            "",
            "PDF selecionado:",
            documentsLabel,
          ].join("\n"),
          irregularidade: [
            "Analise o PDF selecionado e aponte possíveis inconsistências, irregularidades ou pontos que exigem validação.",
            "",
            "PDF selecionado:",
            documentsLabel,
          ].join("\n"),
  ...
  function toggleSelectedPdf(pdfId: string) {
    pdfSelectionInitializedForConversationRef.current =
      ...
      <>
        ...
        <div className="mb-2 flex items-center gap-2">
          ...
          <button
            ...
            className={`inline-flex h-10 shrink-0 items-center justify-center rounded-full text-[10px] font-semibold ${
              options.attachmentButtonIcon ? "w-10 px-0" : "px-4"
            } ${
          ...
          {attachedPdfs.length > 0 && (
            <div ref={quickActionsRef} className="relative shrink-0">
              <button
                type="button"
                disabled={
                  effectiveSelectedPdfIds.length === 0 || isBlocked || accessLoading
                }
                onClick={() =>
                  effectiveSelectedPdfIds.length > 0 &&
              ...
              >
                {effectiveSelectedPdfIds.length > 1
                  ? "AÇÕES RÁPIDAS COM PDFs"
                  : "AÇÕES RÁPIDAS COM PDF"}
              </button>

              {showPdfQuickActions && effectiveSelectedPdfIds.length > 0 && (
                <div
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6">
          {messages.length > 0 ? (
            ...
            <div
              ...
              style={{ color: isStrategic ? "#F5A623" : "#334155" }}
            >
              Carregando conversa...
            </div>
          ) : (
            ...
            {isStrategic
              ? attachedPdfs.length
                ? renderPdfRow({
                  ...
                  quickMenuClassName:
                    "absolute right-0 top-full z-30 mt-2 w-80 rounded-xl border py-2 text-xs text-[#0D2B4D] shadow-lg",
                  quickMenuItemClassName: strategicMenuItemClassName,
                  uploadTextClassName: "mb-2 text-[11px] text-[#5A5A5A]",
            ...
            {isPdfBlockingSend && (
              <div
                className={`mb-2 rounded-xl border px-3 py-2 text-[12px] ${
                  isStrategic
                    ? "border-[#F5A623]/40 bg-[#F5A623]/10 text-amber-950"
  ...
  return (
    <div className="h-screen" style={{ backgroundColor: theme.colors.bg }}>
      <div className="flex h-full flex-col md:hidden">
        <header
          className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between border-b px-4 py-3 shadow-sm"
          style={{
            backgroundColor: isStrategic ? theme.colors.bgSecondary : "#f5f5f5",
            color: isStrategic ? "#0D2B4D" : "#0f172a",
          }}
        >
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div
                ...
                style={{ color: isStrategic ? "#5A5A5A" : "#64748b" }}
              >
                {brand.vendorLabel}
              </div>
            </div>
          ...
          <div className="flex items-center gap-4">
            <span
              className="text-[11px]"
              style={{ color: isStrategic ? "#5A5A5A" : "#475569" }}
            >
            ...
            <button
              ...
              style={{
                borderColor: isStrategic ? "rgba(13,43,77,0.20)" : "#cbd5e1",
                backgroundColor: isStrategic ? "#FFFFFF" : "#ffffff",
              }}
            >
              <span className="flex flex-col gap-[3px]">
                <span
                  className="block h-[2px] w-4 rounded"
                  style={{ backgroundColor: isStrategic ? "#0D2B4D" : "#334155" }}
                />
                <span
                  className="block h-[2px] w-4 rounded"
                  style={{ backgroundColor: isStrategic ? "#0D2B4D" : "#334155" }}
