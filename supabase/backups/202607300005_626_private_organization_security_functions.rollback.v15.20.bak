...
-- Rollback funcional: restaura as implementações SECURITY DEFINER no schema public.
-- Use somente se houver regressão comprovada. Este rollback fará os avisos do
-- Security Advisor retornarem para as funções concedidas a authenticated.
...
create or replace function public.has_organization_role_at_least(
  p_organization_id uuid, p_minimum_role text
)
returns boolean language sql stable security definer
set search_path = pg_catalog, public
as $$
...
begin
  ...
  insert into public.organization_audit_logs (
    ...
    p_organization_id, auth.uid(), 'governance_conversation.created',
    'governance_conversation', v_conversation.id,
    jsonb_build_object(
...
begin
  ...
  insert into public.organizations (
    ...
    p_name, p_legal_name, p_cnpj, p_slug,
    coalesce(p_primary_color, '#0f766e'), auth.uid()
  ) returning id into v_organization_id;
  insert into public.organization_members (
    organization_id, user_id, functional_role, technical_role,
    status, invited_by, joined_at
  ) values (
    v_organization_id, auth.uid(), 'administrador', 'owner',
    'active', auth.uid(), pg_catalog.now()
  );
  insert into public.organization_audit_logs (
    organization_id, actor_user_id, action, entity_type, entity_id, metadata
  ) values (
    v_organization_id, auth.uid(), 'organization.created',
...
revoke all on function public.create_governance_organization(text, text, text, text, text) from public, anon;
...
-- A regra aprovada continua preservada: criação de organização somente backend.
revoke execute on function public.create_governance_organization(text, text, text, text, text) from authenticated;
grant execute on function public.create_governance_organization(text, text, text, text, text) to service_role;

-- Remove o schema privado somente depois de restaurar as funções públicas.
