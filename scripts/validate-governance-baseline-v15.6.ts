...
const baselinePath = path.resolve("docs/governance-baseline-v15.6.json");
...
if (fs.existsSync(baselinePath)) {
  ...
  check(baseline.version === "15.6", "baseline: versão registrada como 15.6");
  check((baseline.scope?.length ?? 0) >= 5, "baseline: escopo funcional documentado");
  check((baseline.limitations?.length ?? 0) >= 2, "baseline: limitações conhecidas documentadas");
}

console.log(`\nGovernança v15.6: ${checks - failures}/${checks} verificações aprovadas.`);
