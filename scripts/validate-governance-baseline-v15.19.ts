...
check(GOVERNANCE_CORE_VERSION === "15.19", "versão central do core é 15.19");
check(
...
check(
  ...
  "idempotência v15.19: banco impede duas respostas do assistente para a mesma requisição",
);
check(
  ...
  "idempotência v15.19: duplicidades históricas são saneadas sem apagar respostas",
);
check(
  ...
  "idempotência v15.19: conflito do assistente recupera a resposta já persistida",
);
check(
  assistantPersistenceSourceV1519.includes("duplicate: true") &&
    assistantPersistenceSourceV1519.includes("duplicate: false"),
  "idempotência v15.19: saver distingue inserção nova de recuperação idempotente",
);
check(
  fs.existsSync(path.resolve("supabase/rollback/202607280001_621_governance_assistant_idempotency.rollback.sql")),
  "idempotência v15.19: migration possui rollback explícito",
);

check(
  GOVERNANCE_CORE_VERSION === "15.19" &&
    GOVERNANCE_PIPELINE_VERSION ===
      "governance-v15.19-assistant-idempotency" &&
    GOVERNANCE_RESULT_SNAPSHOT_VERSION === "15.19",
  "versionamento v15.19: core, pipeline e snapshot estão alinhados",
);

console.log(`\nGovernança v15.19: ${checks - failures}/${checks} verificações aprovadas.`);
