...
const baselinePath = path.resolve("docs/governance-baseline-v15.13.json");
...
if (fs.existsSync(baselinePath)) {
  ...
  check(baseline.version === "15.13", "baseline: versão registrada como 15.13");
...
check(
  ...
  "persistência v15.13: cliente usa o parser compartilhado do snapshot",
);
check(
  metadataBuilder.includes("buildGovernanceResultSnapshot"),
  "persistência v15.13: backend usa o builder compartilhado do snapshot",
);
check(
  GOVERNANCE_RESULT_SNAPSHOT_VERSION === "15.13",
  "persistência v15.13: contrato versionado em 15.13",
);
check(
  GOVERNANCE_PIPELINE_VERSION === "governance-v15.13-canonical-response-envelope",
  "observabilidade v15.13: pipeline identifica conclusão unificada",
);
...
check(
  ...
  "persistência v15.13: builder separa Base legal, evidências e canais",
);
...
check(
  ...
  "persistência v15.13: parser remove duplicações e itens inválidos",
);
...
check(
  ...
  "persistência v15.13: reidratação preserva a categoria de cada referência",
);

check(
  parseGovernanceResultSnapshot({ references: [] }) === null,
  "persistência v15.13: parser não confunde metadata legado com snapshot novo",
);
...
check(GOVERNANCE_CORE_VERSION === "15.13", "versão central do core é 15.13");
check(
...
check(
  ...
  "persistência v15.13: mensagens novas gravam o snapshot canônico",
);
check(
  !metadataSourceV1511.includes("    sources: responseSources,") &&
    (metadataSourceV1511.match(/^    references,$/gm) ?? []).length === 1,
  "persistência v15.13: metadata não grava cópias legadas de fontes e referências",
);
check(
  ...
  "persistência v15.13: arrays legados específicos foram removidos das mensagens novas",
);
check(
  ...
  "persistência v15.13: telemetria mantém apenas contagens diagnósticas",
);
...
check(
  ...
  "resposta transitória v15.13: mensagem não persistida também carrega snapshot canônico",
);
check(
  infrastructureSource.includes("pipeline_version: GOVERNANCE_PIPELINE_VERSION"),
  "resposta transitória v15.13: mensagem não persistida registra a versão do pipeline",
);
check(
  transientResponseSource.includes("references: params.references"),
  "resposta transitória v15.13: referências finalizadas são entregues ao builder da mensagem",
);
check(
  infrastructureSource.includes('evidenceStatus: params.evidenceStatus ?? "transient"'),
  "resposta transitória v15.13: snapshot identifica explicitamente ausência de persistência",
);
check(
  !infrastructureSource.includes("metadata: {\n      source: \"openai\",\n      product_tier: \"governance\",\n      response_mode"),
  "resposta transitória v15.13: metadata não permanece no formato antigo sem snapshot",
);
...
check(
  ...
  "envelope v15.13: resposta persistida não duplica fontes ou referências fora da mensagem",
);
check(
  !transientPayloadSourceV1513.includes("sources: params.sources") &&
    !/\n    references: params\.references,\n    persistenceSkipped/.test(transientPayloadSourceV1513),
  "envelope v15.13: resposta transitória não expõe cópias de referências no nível superior",
);
check(
  ...
  "envelope v15.13: referências transitórias continuam entrando no snapshot da mensagem",
);
check(
  ...
  "envelope v15.13: streaming e não streaming retornam apenas a mensagem canônica",
);

console.log(`\nGovernança v15.13: ${checks - failures}/${checks} verificações aprovadas.`);
