...
const baselinePath = path.resolve("docs/governance-baseline-v15.12.json");
...
if (fs.existsSync(baselinePath)) {
  ...
  check(baseline.version === "15.12", "baseline: versão registrada como 15.12");
...
check(
  ...
  "persistência v15.12: cliente usa o parser compartilhado do snapshot",
);
check(
  metadataBuilder.includes("buildGovernanceResultSnapshot"),
  "persistência v15.12: backend usa o builder compartilhado do snapshot",
);
check(
  GOVERNANCE_RESULT_SNAPSHOT_VERSION === "15.12",
  "persistência v15.12: contrato versionado em 15.12",
);
check(
  GOVERNANCE_PIPELINE_VERSION === "governance-v15.12-canonical-transient-metadata",
  "observabilidade v15.12: pipeline identifica conclusão unificada",
);
...
check(
  ...
  "persistência v15.12: builder separa Base legal, evidências e canais",
);
...
check(
  ...
  "persistência v15.12: parser remove duplicações e itens inválidos",
);
...
check(
  ...
  "persistência v15.12: reidratação preserva a categoria de cada referência",
);

check(
  parseGovernanceResultSnapshot({ references: [] }) === null,
  "persistência v15.12: parser não confunde metadata legado com snapshot novo",
);
...
check(GOVERNANCE_CORE_VERSION === "15.12", "versão central do core é 15.12");
check(
...
check(
  ...
  "persistência v15.12: mensagens novas gravam o snapshot canônico",
);
check(
  !metadataSourceV1511.includes("    sources: responseSources,") &&
    (metadataSourceV1511.match(/^    references,$/gm) ?? []).length === 1,
  "persistência v15.12: metadata não grava cópias legadas de fontes e referências",
);
check(
  ...
  "persistência v15.12: arrays legados específicos foram removidos das mensagens novas",
);
check(
  ...
  "persistência v15.12: telemetria mantém apenas contagens diagnósticas",
);
...
check(
  ...
  "resposta transitória v15.12: mensagem não persistida também carrega snapshot canônico",
);
check(
  infrastructureSource.includes("pipeline_version: GOVERNANCE_PIPELINE_VERSION"),
  "resposta transitória v15.12: mensagem não persistida registra a versão do pipeline",
);
check(
  transientResponseSource.includes("references: params.references"),
  "resposta transitória v15.12: referências finalizadas são entregues ao builder da mensagem",
);
check(
  infrastructureSource.includes('evidenceStatus: params.evidenceStatus ?? "transient"'),
  "resposta transitória v15.12: snapshot identifica explicitamente ausência de persistência",
);
check(
  !infrastructureSource.includes("metadata: {\n      source: \"openai\",\n      product_tier: \"governance\",\n      response_mode"),
  "resposta transitória v15.12: metadata não permanece no formato antigo sem snapshot",
);

console.log(`\nGovernança v15.12: ${checks - failures}/${checks} verificações aprovadas.`);
