...
check(
  ...
  "persistência v15.8: cliente usa o parser compartilhado do snapshot",
);
check(
  metadataBuilder.includes("buildGovernanceResultSnapshot"),
  "persistência v15.8: backend usa o builder compartilhado do snapshot",
);
check(
  snapshotContract.includes('GOVERNANCE_RESULT_SNAPSHOT_VERSION = "15.8"'),
  "persistência v15.8: contrato versionado em 15.8",
);
check(
  observability.includes("governance-v15.8-shared-snapshot-contract"),
  "observabilidade v15.8: pipeline identifica contrato compartilhado",
);
...
check(
  ...
  "persistência v15.8: builder separa Base legal, evidências e canais",
);
...
check(
  ...
  "persistência v15.8: parser remove duplicações e itens inválidos",
);
...
check(
  ...
  "persistência v15.8: reidratação preserva a categoria de cada referência",
);

check(
  parseGovernanceResultSnapshot({ references: [] }) === null,
  "persistência v15.8: parser não confunde metadata legado com snapshot novo",
);

console.log(`\nGovernança v15.8: ${checks - failures}/${checks} verificações aprovadas.`);
