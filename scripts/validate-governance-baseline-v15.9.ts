...
const baselinePath = path.resolve("docs/governance-baseline-v15.8.json");
...
if (fs.existsSync(baselinePath)) {
  ...
  check(baseline.version === "15.8", "baseline: versão registrada como 15.8");
...
check(
  ...
  "persistência v15.9: cliente usa o parser compartilhado do snapshot",
);
check(
  metadataBuilder.includes("buildGovernanceResultSnapshot"),
  "persistência v15.9: backend usa o builder compartilhado do snapshot",
);
check(
  GOVERNANCE_RESULT_SNAPSHOT_VERSION === "15.9",
  "persistência v15.9: contrato versionado em 15.9",
);
check(
  GOVERNANCE_PIPELINE_VERSION === "governance-v15.9-single-query-plan",
  "observabilidade v15.9: pipeline identifica plano único",
);
...
check(
  ...
  "persistência v15.9: builder separa Base legal, evidências e canais",
);
...
check(
  ...
  "persistência v15.9: parser remove duplicações e itens inválidos",
);
...
check(
  ...
  "persistência v15.9: reidratação preserva a categoria de cada referência",
);

check(
  parseGovernanceResultSnapshot({ references: [] }) === null,
  "persistência v15.9: parser não confunde metadata legado com snapshot novo",
);


const routeSourceV159 = fs.readFileSync(
  path.join(process.cwd(), "src/app/api/governance/chat/route.ts"),
  "utf8",
);
const orchestratorSourceV159 = fs.readFileSync(
  path.join(process.cwd(), "src/lib/governance-core/orchestrator.ts"),
  "utf8",
);

check(GOVERNANCE_CORE_VERSION === "15.9", "versão central do core é 15.9");
check(
  routeSourceV159.includes("queryPlan: governancePlan"),
  "rota entrega ao orquestrador o mesmo plano usado nas demais decisões",
);
check(
  !routeSourceV159.includes('pipeline: "governance-core-v15.3"'),
  "rota não registra versão antiga fixa na telemetria",
);
check(
  orchestratorSourceV159.includes("params.queryPlan ?? buildGovernanceV2QueryPlan(params.question)"),
  "orquestrador reutiliza o plano recebido e mantém fallback compatível",
);

console.log(`\nGovernança v15.9: ${checks - failures}/${checks} verificações aprovadas.`);
