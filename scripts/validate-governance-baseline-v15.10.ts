...
const baselinePath = path.resolve("docs/governance-baseline-v15.10.json");
...
if (fs.existsSync(baselinePath)) {
  ...
  check(baseline.version === "15.10", "baseline: versão registrada como 15.10");
...
check(
  ...
  "persistência v15.10: cliente usa o parser compartilhado do snapshot",
);
check(
  metadataBuilder.includes("buildGovernanceResultSnapshot"),
  "persistência v15.10: backend usa o builder compartilhado do snapshot",
);
check(
  GOVERNANCE_RESULT_SNAPSHOT_VERSION === "15.10",
  "persistência v15.10: contrato versionado em 15.10",
);
check(
  GOVERNANCE_PIPELINE_VERSION === "governance-v15.10-unified-completion",
  "observabilidade v15.10: pipeline identifica conclusão unificada",
);
...
check(
  ...
  "persistência v15.10: builder separa Base legal, evidências e canais",
);
...
check(
  ...
  "persistência v15.10: parser remove duplicações e itens inválidos",
);
...
check(
  ...
  "persistência v15.10: reidratação preserva a categoria de cada referência",
);

check(
  parseGovernanceResultSnapshot({ references: [] }) === null,
  "persistência v15.10: parser não confunde metadata legado com snapshot novo",
);
...
check(GOVERNANCE_CORE_VERSION === "15.10", "versão central do core é 15.10");
check(
...
console.log(`\nGovernança v15.10: ${checks - failures}/${checks} verificações aprovadas.`);
