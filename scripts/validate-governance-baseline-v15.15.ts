...
const baselinePath = path.resolve("docs/governance-baseline-v15.15.json");
...
if (fs.existsSync(baselinePath)) {
  ...
  check(baseline.version === "15.15", "baseline: versão registrada como 15.15");
...
check(
  ...
  "persistência v15.15: cliente usa o parser compartilhado do snapshot",
);
check(
  metadataBuilder.includes("buildGovernanceResultSnapshot"),
  "persistência v15.15: backend usa o builder compartilhado do snapshot",
);
check(
  GOVERNANCE_RESULT_SNAPSHOT_VERSION === "15.15",
  "persistência v15.15: contrato versionado em 15.15",
);
check(
  GOVERNANCE_PIPELINE_VERSION === "governance-v15.15-early-persistence-readiness",
  "observabilidade v15.15: pipeline identifica conclusão unificada",
);
...
check(
  ...
  "persistência v15.15: builder separa Base legal, evidências e canais",
);
...
check(
  ...
  "persistência v15.15: parser remove duplicações e itens inválidos",
);
...
check(
  ...
  "persistência v15.15: reidratação preserva a categoria de cada referência",
);

check(
  parseGovernanceResultSnapshot({ references: [] }) === null,
  "persistência v15.15: parser não confunde metadata legado com snapshot novo",
);
...
check(GOVERNANCE_CORE_VERSION === "15.15", "versão central do core é 15.15");
check(
...
check(
  ...
  "persistência v15.15: mensagens novas gravam o snapshot canônico",
);
check(
  !metadataSourceV1511.includes("    sources: responseSources,") &&
    (metadataSourceV1511.match(/^    references,$/gm) ?? []).length === 1,
  "persistência v15.15: metadata não grava cópias legadas de fontes e referências",
);
check(
  ...
  "persistência v15.15: arrays legados específicos foram removidos das mensagens novas",
);
check(
  ...
  "persistência v15.15: telemetria mantém apenas contagens diagnósticas",
);
...
check(
  ...
  "resposta transitória v15.15: mensagem não persistida também carrega snapshot canônico",
);
check(
  infrastructureSource.includes("pipeline_version: GOVERNANCE_PIPELINE_VERSION"),
  "resposta transitória v15.15: mensagem não persistida registra a versão do pipeline",
);
check(
  transientResponseSource.includes("references: params.references"),
  "resposta transitória v15.15: referências finalizadas são entregues ao builder da mensagem",
);
check(
  infrastructureSource.includes('evidenceStatus: params.evidenceStatus ?? "transient"'),
  "resposta transitória v15.15: snapshot identifica explicitamente ausência de persistência",
);
check(
  !infrastructureSource.includes("metadata: {\n      source: \"openai\",\n      product_tier: \"governance\",\n      response_mode"),
  "resposta transitória v15.15: metadata não permanece no formato antigo sem snapshot",
);
...
check(
  ...
  "envelope v15.15: resposta persistida não duplica fontes ou referências fora da mensagem",
);
check(
  !transientPayloadSourceV1513.includes("sources: params.sources") &&
    !/\n    references: params\.references,\n    persistenceSkipped/.test(transientPayloadSourceV1513),
  "envelope v15.15: resposta transitória não expõe cópias de referências no nível superior",
);
check(
  ...
  "envelope v15.15: referências transitórias continuam entrando no snapshot da mensagem",
);
check(
  !routeSourceV1510.includes("sources: finalResponseSources") &&
    (routeSourceV1510.match(/buildGovernancePersistedResponsePayload\(\{/g) ?? []).length === 2,
  "envelope v15.15: streaming e não streaming retornam apenas a mensagem canônica",
);
...
check(
  ...
  "snapshot v15.15: contrato completo e legitimamente vazio permanece válido",
);
...
check(
  ...
  "snapshot v15.15: objeto parcial não bloqueia fallback legado do cliente",
);
...
check(
  ...
  "snapshot v15.15: versão ausente invalida o contrato",
);
...
check(
  ...
  "snapshot v15.15: coleções malformadas invalidam o contrato",
);
...
check(
  ...
  "persistência v15.15: cliente confiável é validado explicitamente",
);
check(
  (routeSourceV1515.match(/const trustedAssistantSupabase = createServiceRoleSupabaseClient\(\);/g) ?? []).length === 1,
  "persistência v15.15: existe uma única criação do cliente confiável por requisição",
);
check(
  trustedPersistenceIndex >= 0 && trustedPersistenceIndex < saveUserMessageIndex,
  "persistência v15.15: service role é validada antes de salvar a mensagem do usuário",
);
check(
  trustedPersistenceIndex >= 0 && trustedPersistenceIndex < officialWebResearchIndex,
  "persistência v15.15: falha de configuração interrompe a requisição antes da pesquisa externa",
);
check(
  routeSourceV1515.indexOf("GOVERNANCE_CHAT_TRUSTED_PERSISTENCE_UNAVAILABLE") < saveUserMessageIndex,
  "persistência v15.15: resposta 503 ocorre antes de qualquer gravação de mensagem",
);

console.log(`\nGovernança v15.15: ${checks - failures}/${checks} verificações aprovadas.`);
